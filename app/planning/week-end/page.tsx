              ...
              {data.items.map((item) => (
                <Card key={`${item.eventType}:${item.eventId}`}>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <Button variant="outline" size="sm" asChild><Link href={`/planning/evenement/${item.eventType}/${encodeURIComponent(item.eventId)}`}>Ouvrir l’espace événement</Link></Button>
                  </CardContent>
                </Card>
              ))}
