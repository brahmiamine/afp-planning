...
export default function MesEchangesPage() {
  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="container mx-auto space-y-6 px-3 py-5 sm:px-4 sm:py-8">
      <PageHeader
        icon={<ArrowLeftRight />}
        title="Échanges d’affectations"
        description="Proposez un échange à une personne disponible. L’échange devient effectif seulement après son accord puis validation d’un administrateur."
      />
      ...
      <section><h3 className="mb-3 flex items-center gap-2 text-lg font-semibold"><span className="flex h-7 w-7 items-center justify-center rounded-md bg-primary-soft text-primary"><ArrowLeftRight className="h-4 w-4" /></span> Propositions reçues</h3><div className="grid gap-3 lg:grid-cols-2">
        {(swaps?.incoming ?? []).length ? swaps?.incoming.map((swap) => <Card key={swap.id}><CardContent className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{swap.payload.eventTitle}</p><p className="text-sm text-muted-foreground">{formatIsoDate(swap.payload.eventDate)} · {swap.payload.eventTime} · {swap.payload.role}</p><p className="text-sm">Proposé par {swap.payload.requester.nom}</p></div><Badge variant="brand">{statusLabels[swap.payload.status]}</Badge></div>{swap.payload.message && <p className="rounded-md bg-secondary-soft p-2 text-sm">{swap.payload.message}</p>}{swap.payload.status === 'pending-target' && <div className="flex gap-2"><Button size="sm" onClick={() => respond(swap.id, 'accept')} disabled={busy}><Check className="mr-1 h-4 w-4" /> Accepter</Button><Button size="sm" variant="destructive" onClick={() => respond(swap.id, 'decline')} disabled={busy}><X className="mr-1 h-4 w-4" /> Refuser</Button></div>}</CardContent></Card>) : <Card><CardContent className="py-8 text-center text-muted-foreground">Aucune proposition reçue.</CardContent></Card>}
      </div></section>

      <section><h3 className="mb-3 flex items-center gap-2 text-lg font-semibold"><span className="flex h-7 w-7 items-center justify-center rounded-md bg-primary-soft text-primary"><ArrowLeftRight className="h-4 w-4" /></span> Mes demandes</h3><div className="grid gap-3 lg:grid-cols-2">
        {(swaps?.mine ?? []).length ? swaps?.mine.map((swap) => <Card key={swap.id}><CardContent className="space-y-3 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-semibold">{swap.payload.eventTitle}</p><p className="text-sm text-muted-foreground">Avec {swap.payload.target.nom} · {formatIsoDate(swap.payload.eventDate)} {swap.payload.eventTime}</p></div><Badge variant={swap.payload.status === 'approved' ? 'default' : 'outline'}>{statusLabels[swap.payload.status]}</Badge></div>{(swap.payload.status === 'pending-target' || swap.payload.status === 'pending-admin') && <Button size="sm" variant="outline" onClick={() => cancel(swap.id)} disabled={busy}>Annuler la demande</Button>}</CardContent></Card>) : <Card><CardContent className="py-8 text-center text-muted-foreground">Aucune demande d’échange.</CardContent></Card>}
      </div></section>
    </main>
  </div>;
}