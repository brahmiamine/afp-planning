...
export function AvailabilityCampaignsView({ mode, refreshKey = 0 }: { mode: 'manage' | 'personal'; refreshKey?: number }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await apiGet<{ requests: AvailabilityCampaign[]; responses: AvailabilityResponse[] }>(
        `/api/availability-requests${mode === 'personal' ? '?scope=personal' : ''}`,
      );
        ...
        {editable && (
          <Card>
            ...
            <CardContent className="grid gap-3 md:grid-cols-2">
              ...
              <label className="text-sm md:col-span-2">Clôture des réponses (optionnel — sinon fin de période)
