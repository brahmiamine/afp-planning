...
export const MatchDetails = memo(function MatchDetails({ match, extras }: MatchDetailsProps) {
  ...
  return (
    <div className="border-t pt-3 sm:pt-4 space-y-2 sm:space-y-3">
      ...
      {extras &&
        (hasContactData(extras.arbitreTouche) || hasContactData(extras.contactEncadrants) || hasContactData(extras.contactAccompagnateur)) && (
          <div className="border-t pt-3 mt-3 bg-muted rounded-lg p-3">
            ...
            <div className="text-sm text-foreground space-y-1.5">
              {/* Arbitres AFP */}
              {Array.isArray(extras.arbitreTouche) &&
                extras.arbitreTouche.length > 0 &&
                extras.arbitreTouche.map((arbitre, index) => (
              ...
              {/* Encadrants */}
              {Array.isArray(extras.contactEncadrants) &&
                extras.contactEncadrants.length > 0 &&
                extras.contactEncadrants.map((encadrant, index) => (
                  <div key={index} className="flex items-start gap-2">
                    ...
                    <span>
                      ...
                      {encadrant.numero && <span className="text-muted-foreground"> - {encadrant.numero}</span>}
                    </span>
                  </div>
                ))}
              {(() => {
                  ...
                  oldEncadrant && (
                    <div className="flex items-start gap-2">
                      ...
                      <span>
                        ...
                        {oldEncadrant.numero && <span className="text-muted-foreground"> - {oldEncadrant.numero}</span>}
                      </span>
                    </div>
                  )
                );
              ...
              {/* Accompagnateurs */}
              {Array.isArray(extras.contactAccompagnateur) &&
                extras.contactAccompagnateur.length > 0 &&
                extras.contactAccompagnateur.map((accompagnateur, index) => (
                  <div key={index} className="flex items-start gap-2">
                    <Phone className="w-3.5 h-3.5 text-primary mt-0.5 shrink-0" />
                    <span>
                      Accompagnateur {extras.contactAccompagnateur!.length > 1 ? `#${index + 1}` : ""}:{" "}
                      <span className="font-medium">{accompagnateur.nom}</span>
                      {accompagnateur.numero && <span className="text-muted-foreground"> - {accompagnateur.numero}</span>}
                    </span>
                  </div>
                ))}
              {(() => {
                  ...
                  oldAccompagnateur && (
                    <div className="flex items-start gap-2">
                      ...
                      <span>
                        ...
                        {oldAccompagnateur.numero && <span className="text-muted-foreground"> - {oldAccompagnateur.numero}</span>}
                      </span>
                    </div>
                  )
                );
