...
| `User` | `roles` | Oui | Rare | Non — 3 valeurs possibles (`admin`/`arbitre`/`encadrant`/`accompagnateur`), filtré en mémoire sur de petits volumes | Non, sauf si le volume d'utilisateurs par club devient très grand |
...
## 3. Prochaine étape concrète proposée (hors périmètre de ce ticket)

Avant de toucher au schéma des 5 tables d'événements, introduire un codec de lecture/
écriture pour `MatchExtras` (le candidat n°1 ci-dessus) :

```ts
// Proposition — non implémentée ici, pour dimensionner un futur ticket.
interface MatchExtrasEnvelope {
  schemaVersion: 1;
  data: MatchExtras;
}

function parseMatchExtrasPayload(raw: unknown, matchId: string): MatchExtras {
  // raw peut être : (a) un payload historique sans schemaVersion (avant ce codec),
  // (b) une MatchExtrasEnvelope versionnée. Les deux doivent être acceptés en lecture ;
  // seule l'écriture produit désormais la forme versionnée.
}
```

Une fois ce codec en place et couvert par des tests (payloads historiques ET
nouveaux), sortir `planningStatus`/`planningRevision` en colonnes réelles sur
`MatchExtra` devient une migration additive classique (nouvelle colonne, backfill
depuis le JSON, bascule des lectures, dépréciation du champ JSON) — le même schéma que
la migration 0009 (`audit_log_tenant_scoped`) déjà utilisée dans ce dépôt.

Ne pas faire porter cette migration par le présent ticket : elle touche l'écriture de
chaque événement (publication, scraper, échanges, présence) et mérite sa propre revue
et son propre passage de tests d'injection d'échec, dans la continuité de l'atomicité
apportée par l'issue #152.

## Suivi

- Cette cartographie doit être mise à jour à chaque nouvelle colonne `simple-json` ou
  nouveau `kind` de `planning_records`.
- Toute normalisation partant de ce document doit respecter les critères d'acceptation
  de l'issue #128 : migrations rétrocompatibles, tests sur données historiques et
  nouvelles, aucune régression sur publication/scraper/historique.
